...
rule operatorCommissionUpdatesOnDelay(env e1, env e2) {

    address operator;
    IAVSDirectory.OperatorSet operatorSet;
    IRewardsCoordinator.RewardType rewardType;
    IRewardsCoordinator.OperatorCommissionUpdate[] operatorCommissionUpdate =
        operatorCommissionUpdatesArray(operator, operatorSet.avs, operatorSet.operatorSetId, rewardType);

    require operatorCommissionUpdate[0].effectTimestamp == e1.block.timestamp
        && operatorCommissionUpdate[1].effectTimestamp == e2.block.timestamp;
    require e1.block.timestamp < e2.block.timestamp;
    require operatorCommissionUpdate[0].bips != operatorCommissionUpdate[1].bips;

    uint16 beforeBips = getOperatorCommissionBips(e1, operator, operatorSet, rewardType);
    uint16 afterBips = getOperatorCommissionBips(e2, operator, operatorSet, rewardType);

    assert beforeBips != afterBips;
}